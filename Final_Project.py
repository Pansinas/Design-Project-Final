...
set_background("C:/Users/kumar/OneDrive/Documents/streamlit/transulater/finalpp.jpg")      #paste image path 
### transator
...
if st.button("Record"):
    if not st.session_state["is_recording"]:
        ...
        st.session_state["is_recording"] = False  # Stop recording after storing input

st.write('---')

# Text input with prefilled value
type_text = st.text_input("Enter a sentence: 👇", value=st.session_state["input_text"])
if type_text:
    st.session_state["final_sentence"] = type_text  # Use typed text if available
elif st.session_state["input_text"]:
    st.session_state["final_sentence"] = st.session_state["input_text"]  # Use voice input if available
else:
    st.session_state["final_sentence"] = ""  # Or handle accordingly

st.write("Final Input Sentence: ", st.session_state["final_sentence"])  # Display the final sentence # final text 
